  ...
  return (
    ...
    >
      ...
      <div className="z-10 flex flex-col items-center justify-center flex-1 w-full gap-2">
        <h2 className="font-serif w-full drop-shadow-md" style={{ color: data.colors.accent, fontSize: 36 + _d }}>{data.title}</h2>
        <div className="h-4 w-px my-1 opacity-60" style={{ backgroundColor: data.colors.secondary }} />
        <h2 className="font-serif w-full drop-shadow-md" style={{ color: data.colors.accent, fontSize: 36 + _d }}>{data.subtitle}</h2>
      </div>
      ...
      <div className="z-10 mt-auto mb-8 w-full">
        ...
        <p className="font-serif leading-relaxed max-w-[200px] mx-auto opacity-90" style={{ fontSize: 12 + _d }}>{data.description}</p>
      </div>
    </div>
  );
}
