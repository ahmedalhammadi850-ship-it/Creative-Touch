  ...
  return (
    <div id="template-preview" style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '0 22px' }}>
        ...
        <div style={{ color: '#fff', fontSize: '22px', fontWeight: 900, lineHeight: 1.1, marginBottom: '3px', textShadow: `0 0 20px ${data.colors.accent}66` }}>{data.title}</div>
        <div style={{ color: data.colors.accent, fontSize: '9.5px', fontWeight: 700, marginBottom: '10px' }}>{data.subtitle}</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {data.phone && <span style={{ color: 'rgba(255,255,255,0.65)', fontSize: '7.5px' }} dir="ltr">{data.phone}</span>}
          {data.email && <span style={{ color: 'rgba(255,255,255,0.55)', fontSize: '7.5px' }} dir="ltr">{data.email}</span>}
          {data.website && <span style={{ color: 'rgba(255,255,255,0.55)', fontSize: '7.5px' }} dir="ltr">{data.website}</span>}
        </div>
      </div>
    </div>
  );
