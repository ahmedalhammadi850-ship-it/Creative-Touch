  ...
  return (
    <div id="template-preview" style={{ width: '340px', height: '220px', position: 'relative', overflow: 'hidden', fontFamily: "'Helvetica Neue', Arial, sans-serif", direction: 'rtl', backgroundColor: data.colors.bg }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, width: '42%', height: '100%', padding: '16px 12px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 1 }}>
        ...
        <div style={{ color: data.colors.accent, fontSize: (ffs.company ?? 8) + _d, textAlign: 'center', letterSpacing: '0.1em', opacity: 0.9 }}>{data.description}</div>
      </div>
      ...
      <div style={{ position: 'absolute', top: 0, right: 0, width: '58%', height: '100%', padding: '16px 18px 16px 8px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', zIndex: 1 }}>
        <div>
          ...
          <div style={{ color: data.colors.secondary, fontSize: (ffs.jobTitle ?? 11) + _d, marginTop: '3px', opacity: 0.7 }}>{data.subtitle}</div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }} dir="ltr">
            {data.phone && <span style={{ textAlign: 'right', color: data.colors.secondary, fontSize: (ffs.contact ?? 9) + _d, opacity: 0.65 }}>{data.phone}</span>}
            {data.email && <span style={{ textAlign: 'right', color: data.colors.secondary, fontSize: (ffs.contact ?? 9) + _d, opacity: 0.65 }}>{data.email}</span>}
            {data.website && <span style={{ textAlign: 'right', color: data.colors.secondary, fontSize: (ffs.contact ?? 9) + _d, opacity: 0.65 }}>{data.website}</span>}
          </div>
        </div>
      </div>
    </div>
