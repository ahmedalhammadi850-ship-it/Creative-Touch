  ...
  return (
    <div id="template-preview" style={{ width: '340px', height: '220px', position: 'relative', overflow: 'hidden', fontFamily: "'Helvetica Neue', Arial, sans-serif", direction: 'rtl', background: `linear-gradient(135deg, ${data.colors.primary} 0%, ${data.colors.secondary} 100%)` }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '22px 24px', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div>
          <div style={{ color: data.colors.accent, fontSize: ffs.company ?? 10, letterSpacing: '0.2em', marginBottom: '4px', opacity: 0.85 }}>{data.description}</div>
          <div style={{ color: '#ffffff', fontSize: ffs.name ?? 24, fontWeight: '800', lineHeight: 1.1 }}>{data.title}</div>
          <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: ffs.jobTitle ?? 12, marginTop: '3px' }}>{data.subtitle}</div>
        </div>
        <div style={{ display: 'flex', gap: '14px', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexDirection: 'column' }} dir="ltr">
            {data.phone && <span style={{ textAlign: 'right', fontSize: ffs.contact ?? 9, color: 'rgba(255,255,255,0.7)' }}>{data.phone}</span>}
            {data.email && <span style={{ textAlign: 'right', fontSize: ffs.contact ?? 9, color: 'rgba(255,255,255,0.7)' }}>{data.email}</span>}
            {data.website && <span style={{ textAlign: 'right', fontSize: ffs.contact ?? 9, color: 'rgba(255,255,255,0.7)' }}>{data.website}</span>}
          </div>
        </div>
      </div>
