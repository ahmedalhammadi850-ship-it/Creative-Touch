    ...
    >
      ...
      <div className="z-10 flex justify-between items-start">
        <div>
          ...
          <p className="mt-0.5 opacity-80" style={{ color: data.colors.secondary, fontSize: ffs.jobTitle ?? 12 }}>{data.subtitle}</p>
        </div>
      </div>

      <div className="z-10">
        <p className="font-medium mb-2 opacity-70" style={{ color: data.colors.secondary, fontSize: ffs.company ?? 10 }}>{data.description}</p>
        <div className="flex flex-col gap-0.5">
          {data.phone && <p dir="ltr" className="opacity-75" style={{ color: data.colors.secondary, fontSize: ffs.contact ?? 9 }}>{data.phone}</p>}
          {data.email && <p dir="ltr" className="opacity-75" style={{ color: data.colors.secondary, fontSize: ffs.contact ?? 9 }}>{data.email}</p>}
          {data.website && <p dir="ltr" className="opacity-75" style={{ color: data.colors.secondary, fontSize: ffs.contact ?? 9 }}>{data.website}</p>}
        </div>
