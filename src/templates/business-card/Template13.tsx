  ...
  return (
    <div id="template-preview" style={{
      ...
      <div style={{ flex: 1, padding: '18px 18px', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: data.colors.bg }}>
        ...
        <div style={{ color: data.colors.secondary, fontSize: (ffs.name ?? 19) + _d, fontWeight: 900, lineHeight: 1.15, marginBottom: '3px' }}>{data.title}</div>
        <div style={{ color: data.colors.primary, fontSize: (ffs.jobTitle ?? 9) + _d, fontWeight: 700, marginBottom: '9px', opacity: 0.9 }}>{data.subtitle}</div>
        <div style={{ color: data.colors.secondary, fontSize: (ffs.company ?? 8) + _d, opacity: 0.55, marginBottom: '8px' }}>{data.description}</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {data.phone && <span style={{ color: data.colors.primary, fontSize: (ffs.contact ?? 7.5) + _d, opacity: 0.85, fontWeight: 600 }} dir="ltr">{data.phone}</span>}
          {data.email && <span style={{ color: data.colors.primary, fontSize: (ffs.contact ?? 7.5) + _d, opacity: 0.7 }} dir="ltr">{data.email}</span>}
          {data.website && <span style={{ color: data.colors.primary, fontSize: (ffs.contact ?? 7.5) + _d, opacity: 0.7 }} dir="ltr">{data.website}</span>}
        </div>
      </div>
    </div>
  );
