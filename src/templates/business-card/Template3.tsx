  ...
  return (
    ...
    >
      ...
      <div className="z-10 flex-1 pl-4">
        <h2 className="font-black mb-1" style={{ fontSize: ffs.name ?? 24 }}>{data.title}</h2>
        <p className="opacity-90 font-medium" style={{ fontSize: ffs.jobTitle ?? 12 }}>{data.subtitle}</p>
        <div className="mt-4 w-8 h-1 rounded" style={{ backgroundColor: data.colors.accent }} />
        <p className="mt-3 font-bold opacity-90" style={{ fontSize: ffs.company ?? 12 }}>{data.description}</p>
      </div>

      <div className="z-10 space-y-1.5 text-left border-l pl-3" style={{ borderColor: `${data.colors.bg}40` }}>
        {data.phone && <p dir="ltr" style={{ fontSize: ffs.contact ?? 9 }}>{data.phone}</p>}
        {data.email && <p dir="ltr" style={{ fontSize: ffs.contact ?? 9 }}>{data.email}</p>}
        {data.website && <p dir="ltr" style={{ fontSize: ffs.contact ?? 9 }}>{data.website}</p>}
      </div>
    </div>
  );
}
