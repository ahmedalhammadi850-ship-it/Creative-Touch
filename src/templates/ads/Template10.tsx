    ...
    <div id="template-preview" style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '0 30px' }}>
        <div style={{ display: 'inline-block', background: `${data.colors.primary}33`, border: `1px solid ${data.colors.primary}88`, borderRadius: '20px', padding: '3px 14px', marginBottom: '14px' }}>
          <span style={{ color: data.colors.accent, fontSize: '9px', letterSpacing: '0.2em', fontWeight: 700 }}>عرض خاص</span>
        </div>

        <div style={{ color: data.colors.secondary, fontSize: '26px', fontWeight: 900, lineHeight: 1.2, marginBottom: '8px', textShadow: `0 0 30px ${data.colors.secondary}44` }}>
          {data.title}
        </div>
        ...
        <div style={{ color: data.colors.primary, fontSize: '12px', fontWeight: 700, marginBottom: '10px' }}>{data.subtitle}</div>
        <div style={{ color: `${data.colors.secondary}bb`, fontSize: '10px', lineHeight: 1.7, marginBottom: '20px' }}>{data.description}</div>
