  ...
  return (
    <div id="template-preview" style={{ width: '360px', height: '360px', position: 'relative', overflow: 'hidden', fontFamily: "'Helvetica Neue', Arial, sans-serif", direction: 'rtl', background: `linear-gradient(160deg, ${data.colors.bg} 0%, ${data.colors.primary}22 100%)` }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px 28px', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', textAlign: 'center' }}>
        {/* Stars rating */}
        <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', color: data.colors.accent, fontSize: `${16 + _d}px` }}>
          {[0,1,2,3,4].map(i => <span key={i}>★</span>)}
        </div>

        {/* Main content */}
        <div>
          <div style={{ color: data.colors.primary, fontSize: `${13 + _d}px`, fontWeight: '600', marginBottom: '8px', opacity: 0.7 }}>{data.subtitle}</div>
          <div style={{ color: data.colors.primary, fontSize: `${30 + _d}px`, fontWeight: '900', lineHeight: 1.1, marginBottom: '10px' }}>{data.title}</div>
          ...
          <div style={{ color: data.colors.secondary, fontSize: `${12 + _d}px`, lineHeight: 1.7, maxWidth: '260px', opacity: 0.8 }}>{data.description}</div>
        </div>

        {/* CTA button */}
        {data.phone && (
          <div style={{ background: `linear-gradient(135deg, ${data.colors.primary}, ${data.colors.secondary})`, color: '#ffffff', padding: '10px 28px', borderRadius: '30px', fontSize: `${13 + _d}px`, fontWeight: '800', boxShadow: `0 4px 16px ${data.colors.primary}44`, whiteSpace: 'nowrap' }} dir="ltr">
            {data.phone}
          </div>
        )}
      </div>
