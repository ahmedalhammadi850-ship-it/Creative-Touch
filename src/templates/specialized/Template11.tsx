  ...
  return (
    <div id="template-preview" style={{
      ...
      <div style={{
        ...
        <div style={{ color: colors.accent, fontSize: `${7.5 + _d}px`, letterSpacing: '0.22em', fontWeight: 700, marginBottom: '7px', opacity: 0.9 }}>✦ خدمات متميزة ✦</div>
        <div style={{ color: '#fff', fontSize: `${18 + _d}px`, fontWeight: 900, lineHeight: 1.25, marginBottom: '3px', textShadow: `0 0 14px ${colors.primary}` }}>{title}</div>
        <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: `${10 + _d}px`, marginBottom: '14px' }}>{subtitle}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '14px' }}>
          {services.slice(0, 4).map((s, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.88)', fontSize: `${11 + _d}px`, fontWeight: 600 }}>{s}</span>
            </div>
          ))}
        </div>
        {phone && (
          <div style={{
            ...
            <span style={{ color: '#fff', fontSize: `${12 + _d}px`, fontWeight: 800, textShadow: `0 0 8px ${colors.accent}` }} dir="ltr">{phone}</span>
          </div>
        )}
      </div>
    </div>
